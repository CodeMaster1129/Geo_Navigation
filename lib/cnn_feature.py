...
def cnn_feature_extract(image, scales=[
 0.25, 0.5, 1.0], nfeatures=1000, model_type="swin", model_file=None):
    if model_type == "swin":
    ...
    keypoints[(None[:None], 0)] *= fact_i
    keypoints[(None[:None], 1)] *= fact_j
    keypoints = keypoints[(None[:None], [1, 0, 2])]
    if nfeatures != -1:
        scores2 = np.array([scores]).T
        res = np.hstack((scores2, keypoints))
        res = res[np.lexsort(-res[(None[:None], None[None:-1])].T)]
        res = np.hstack((res, descriptors))
        scores = res[(0[:nfeatures], 0)].copy()
        keypoints = res[(0[:nfeatures], 1[:4])].copy()
        descriptors = res[(0[:nfeatures], 4[:None])].copy()
        del res
    return (
